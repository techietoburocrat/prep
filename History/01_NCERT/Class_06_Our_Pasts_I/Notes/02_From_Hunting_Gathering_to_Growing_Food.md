...
[← Ch.1](01_What_Where_How_and_When.md) | [Index](00_Index.md) | [Next: Ch.3 →](03_In_the_Earliest_Cities.md)
...
| **Prelims (GS Paper I)** | *History of India* → Stone Age cultures, Neolithic sites, prehistoric art |
...
```
  If all of human existence were ONE 24-HOUR DAY:

  00:00 ─────────────────────────────────────────────────────────────► 24:00
  │                                                                      │
  │◄──────────────── HUNTING & GATHERING (23 hrs 53 min) ──────────────►│
  │                                                                      │
  │                                                          FARMING ───►│
  │                                                     starts at 23:53  │
  │                                                                      │
...
```
  ...
  │                                                                  │
  │  Stateless                       →     Stateful                  │
  │  (carry nothing, own nothing)          (granary = persistent     │
  │                                         storage on disk)         │
  │                                                                  │
  │  Ephemeral / autoscaling         →     Pinned to a host          │
  │  (move when resources deplete)         (you CANNOT move — your   │
  │                                         crop is in the ground)   │
  │                                                                  │
  │  Small bands, no hierarchy       →     Large settlements,        │
  │  (flat team, ~25 people)               specialised roles,        │
  │                                         management layer         │
  │                                                                  │
  │  No surplus                      →     SURPLUS                   │
  │  (consume what you gather)             (which someone must       │
  │                                         guard, count, and        │
  │                                         eventually TAX)          │
  │                                                                  │
  │  Generalists                     →     Specialists               │
  │  (everyone does everything)            (potter, weaver, priest,  │
  │                                         soldier, scribe)         │
  └──────────────────────────────────────────────────────────────────┘
```

> 🔑 **The whole causal chain in one line:**
> **Storage → surplus → specialists → hierarchy → the state.**
>
...
## 1️⃣ Life Before Farming: The Hunter-Gatherers
...
> ⚠️ Remember Chapter 1's survival-bias lesson: we call it the "Stone Age" because **stone survived**, not because stone was the main material. Their world was probably mostly wood, fibre and leather — all of which rotted away.
...
```
  ┌───────────────────────┐   ┌───────────────────────┐   ┌───────────────────────┐
  │   HABITATION SITE     │   │    FACTORY SITE       │   │  HABITATION-CUM-      │
  │                       │   │                       │   │  FACTORY SITE         │
  │  Where people LIVED   │   │  Where raw stone was  │   │                       │
  │                       │   │  available and tools  │   │  People both LIVED    │
  │  Find: hearths, food  │   │  were MADE            │   │  and MADE tools here  │
  │  remains, huts, waste │   │                       │   │                       │
  │                       │   │  Find: waste flakes,  │   │  Find: BOTH sets of   │
  │                       │   │  half-finished tools, │   │  evidence together    │
  │                       │   │  NO living debris     │   │                       │
  └───────────────────────┘   └───────────────────────┘   └───────────────────────┘
```

Factory sites cluster near **rivers and streams**, because that's where usable stone (quartzite pebbles) was found.

---

## 2️⃣ The Three Stone Ages — Learn This Table Cold

This is the highest-value table in the chapter.

| | **PALAEOLITHIC** 🪨 | **MESOLITHIC** 🔪 | **NEOLITHIC** 🌾 |
|---|---|---|---|
| **Meaning** | Greek *palaeo* = old, *lithos* = stone → **Old Stone Age** | *meso* = middle → **Middle Stone Age** | *neo* = new → **New Stone Age** |
...
```
        BEFORE (Palaeolithic)              AFTER (Mesolithic)
        ─────────────────────              ──────────────────
        ...
        held directly in the hand          ┌──────────────────────────┐
           ...
           ╲  ███  ╱
            ╲_███_╱                        → SICKLES for cutting grass/grain
                                           → ARROWS and SPEARS (light, fast)
        Heavy. Slow. One function.         → SAWS and knives
        Blunt = throw the whole tool away.
                                           Modular. Light. Replaceable.
                                           One blade chips? Swap it out.
```

> 💻 **This is modularity and component reuse.** The Palaeolithic hand-axe is a monolith — one giant object doing one job, and when part of it fails you discard the whole thing. Microliths are **composable components with a standard interface** (the haft). Same blades, different assemblies: sickle, arrow, spear, saw. And **repair becomes cheap** — replace the failing component, keep the system.
>
...
## 3️⃣ The Neolithic Revolution — How Farming Began
...
## 4️⃣ The Great Sites — Learn These as Pairs
...
```
   ...
   │                          ╱╲╱╲ HIMALAYAS ╱╲╱╲                       │
   │        ★ BURZAHOM (Kashmir)                                        │
   │        ★ GUFKRAL (Kashmir)          pit-dwellings, dog buried      │
   │                                     with master                    │
   │   ★ MEHRGARH                                    ★ DAOJALI HADING   │
   │   (Bolan Pass, Baluchistan)                       (Assam, NE)      │
   │   wheat, barley, cotton,             ★ CHIRAND     jadeite, stone  │
   │   sheep, goat, CATTLE                  (Bihar)     mortars         │
   │   burials w/ goats                     bone tools                  │
   │                                                                    │
   │            ★ BAGOR (Raj.)   ★ KOLDIHWA & MAHAGARA (UP)             │
   │              Mesolithic       ★ SARAI NAHAR RAI    RICE — earliest │
   │                                                                    │
   │      ▓▓▓ VINDHYAS ▓▓▓  ★ BHIMBETKA (MP) — rock art                 │
   │                        ★ ADAMGARH (MP) — Mesolithic, animal bones  │
   │                                                                    │
   │              ★ HUNSGI (Karnataka) — Palaeolithic                   │
   │              ★ HALLUR (Karnataka) — millets, ash mounds            │
   │              ★ PAIYAMPALLI (Tamil Nadu) — millets, pulses          │
   │              ★ KURNOOL CAVES (AP) — evidence of FIRE               │
   │                                                                    │
   └────────────────────────────────────────────────────────────────────┘
```
...
| **Hallur** | Karnataka | Neolithic | **Millets**; South Indian **ash mounds** |
| **Paiyampalli** | Tamil Nadu | Neolithic | **Millets and pulses** |
...
## 5️⃣ What Changed When People Settled Down
...
### ⚖️ The honest trade-off — and why UPSC likes this angle
...
## 6️⃣ Tribal Societies — The Living Link
...
1. Consider the following statements about Burzahom:
   ...
   3. Dogs were buried along with their masters.
   How many are correct? → *All three.*
...
2. Which of the following pairs is/are correctly matched?
   ...
   3. Paiyampalli — cotton cultivation
   → *1 and 2 only. Paiyampalli = millets and pulses; cotton is Mehrgarh.*

3. The defining tool type of the Mesolithic period was —
   → *Microliths.*
...
4. Consider the following:
   ...
   3. Mehrgarh is located near the Khyber Pass.
   → *1 and 2 only. Mehrgarh is on the **Bolan** Pass, not the Khyber.*
...
[← Ch.1](01_What_Where_How_and_When.md) | [Index](00_Index.md) | [Next: Ch.3 — In the Earliest Cities →](03_In_the_Earliest_Cities.md)
